...
<head>
    ...
    <style>
        ...
        /* Match the personal profile workspace and keep viewed profiles scrollable. */
        html,
        body {
            min-height: 100%;
            height: auto;
        ...
        .other-profile-page .profile-hero {
            grid-template-columns: minmax(250px, 0.8fr) minmax(0, 1.7fr);
        }

        .other-profile-page .profile-about .bio-text {
        ...
        .other-post .item-info small {
            display: block;
            color: #fff !important;
            font-size: 11px;
        }

        .other-post:hover .item-img {
            transform: scale(1.04);
        }

        .other-post:hover .item-info,
        .other-post:focus-within .item-info {
            opacity: 1;
...
<body>
    ...
    <main class="profile-page other-profile-page">
        <section class="profile-hero">
            <div class="profile-identity">
                ...
                <div>
                    ...
                    <p class="profile-handle">@<?php echo htmlspecialchars($targetUser); ?></p>
                </div>
            </div>
            ...
            <div class="profile-about">
                ...
                <p class="bio-text">
                    <?php echo htmlspecialchars($user_data['bio'] ?: 'No bio available yet.'); ?>
                </p>
            </div>
        </section>

        <section class="card other-products-card">
            <div class="other-products-heading">
                <h2>Posts by <?php echo htmlspecialchars($user_data['name']); ?></h2>
                <p>Tap a post to view its details</p>
            </div>
            ...
            ?>
                <div class="other-empty">No listings available yet.</div>
            <?php else: ?>
                <div class="other-posts-grid">
                    ...
                    ?>
                        <button
                            type="button"
                            class="item-card other-post"
                            onclick='openItem(<?php echo $json_data; ?>)'
                            aria-label="View <?php echo htmlspecialchars($row['title']); ?>"
                        >
                            <span class="card-tag"><?php echo htmlspecialchars($row['category']); ?></span>
                            <span class="item-img-wrap">
                                <img
                                    src="<?php echo '../image.php?entity=post&amp;id=' . (int)$row['id']; ?>"
                                    class="item-img"
                            ...
                            <span class="item-info">
                                <span>
                                    ...
                                    <small><?php echo ($row['type'] == 'Sell' ? htmlspecialchars($row['price']).' JD' : '🔄 Swap'); ?></small>
                                </span>
                            </span>
                        </button>
                    <?php endwhile; ?>
    ...
    function openItem(post) {
        ...
        document.getElementById('details-view').innerHTML = `
            <img src="../image.php?entity=post&id=${Number(post.id)}" style="width:100%; border-radius:18px; margin-bottom:20px; max-height:350px; object-fit:contain; background:#f1f5f9;" alt="">
